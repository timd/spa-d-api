...
export const required = value => (!value || !value.trim().length ? 'Required' : '')

export const emailFormat = value => (emailRegex.test(value) ? '' : 'Invalid email address format')

export const positiveValue = value => (value > 0 ? '' : 'Value must be positive number')
